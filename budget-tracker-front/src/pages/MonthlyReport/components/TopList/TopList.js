...
const TopList = ({ title, dataMap, labels, totalIncome }) => {
  ...
  return (
    <div className={styles['toplist-card']}>
      <h4>{title}</h4>
      {entries.length===0 ? <p className={styles.empty}>Немає даних</p> :
        <ul className={styles.toplist}>
          {entries.map(([id,sum],i)=>(
            <li key={id}>
              <span className={styles['color-dot']} style={{background:palette[i%palette.length]}}/>
              <span className={styles.name}>{labels[id]||'—'}</span>
              <span className={styles.amount}>{format(sum)}</span>
              <span className={styles.percent}>{((sum/totalIncome)*100).toFixed(1)} %</span>
            </li>
          ))}
        </ul>
      }
